...
async function fetchDashboardData() {
  const [announcementsRes, clientsRes, statsRes, progressRes, queueRes, tokenRes, projectsRes, topicsRes] = await Promise.all([
    ...
    api.get("/scrapers/bk/queue-status"),
    api.get<TokenStats>("/logs/token-stats").catch(() => ({ data: null })),
    api.get<{ meta: { total: number } }>("/clients/all-projects?limit=1").catch(() => ({ data: { meta: { total: 0 } } })),
    api.get<{ meta: { total: number } }>("/clients/all-topics?limit=1").catch(() => ({ data: { meta: { total: 0 } } })),
  ]);
...
function JobTypeRow({
  ...
  label: string;
  stats: { total: number; completed: number; failed: number; running: number; lastRunAt: string | null; avgDurationMs: number | null };
}) {
  ...
  return (
    <div className="flex flex-col gap-2 py-3 first:pt-0 last:pb-0">
      ...
      <div className="flex flex-wrap gap-4 text-xs text-muted-foreground">
        ...
        {successRate !== null && (
          <span className="ml-auto font-medium text-foreground">{successRate}% sukces</span>
        )}
      </div>
    </div>
  );
...
const ANALYSIS_COST_PER_1M_OUT = 0.60;
      ...
      <CardContent className="space-y-4">
        <div>
          <div className="mb-2 flex items-center justify-between">
            ...
            <Badge variant="secondary" className="font-mono text-xs">{EMBEDDING_MODEL}</Badge>
          </div>
        ...
        <div>
          <div className="mb-2 flex items-center justify-between">
            ...
            <Badge variant="secondary" className="font-mono text-xs">{ANALYSIS_MODEL}</Badge>
          </div>
          <dl className="grid grid-cols-2 gap-x-4 gap-y-1 text-xs">
            ...
            <dd className="font-mono">${ANALYSIS_COST_PER_1M_OUT}/1M tokenów</dd>
            <dt className="text-muted-foreground">Provider</dt>
            <dd>OpenAI</dd>
          </dl>
        </div>
...
function calcCostUsd(promptTokens: number, completionTokens: number): number {
  return (promptTokens / 1_000_000) * LLM_COST_IN + (completionTokens / 1_000_000) * LLM_COST_OUT;
}
  ...
  if (!tokenStats) {
    return (
      <Card>
        ...
        <CardContent>
          <p className="text-xs text-muted-foreground">Brak danych — tokeny będą zliczane od nowych jobów.</p>
        </CardContent>
      </Card>
    );
  }

  const estimatedCost = calcCostUsd(tokenStats.promptTokens, tokenStats.completionTokens);
      ...
      <CardContent className="space-y-4">
        <div className="grid grid-cols-3 gap-3">
          <div className="rounded-lg bg-muted/50 p-3 text-center">
            ...
            <p className="mt-0.5 text-lg font-bold tabular-nums">{fmtTokens(tokenStats.promptTokens)}</p>
          </div>
          <div className="rounded-lg bg-muted/50 p-3 text-center">
            <p className="text-xs text-muted-foreground">Output</p>
            <p className="mt-0.5 text-lg font-bold tabular-nums">{fmtTokens(tokenStats.completionTokens)}</p>
          </div>
          <div className="rounded-lg bg-muted/50 p-3 text-center">
            <p className="text-xs text-muted-foreground">Łącznie</p>
            <p className="mt-0.5 text-lg font-bold tabular-nums">{fmtTokens(tokenStats.totalTokens)}</p>
          </div>
        </div>
        ...
        {(embeddingType || reportType) && (
          <>
            ...
            <div className="space-y-1.5 text-xs">
              {embeddingType && (
                <div className="flex items-center justify-between">
                  <span className="text-muted-foreground">Embedding (analiza)</span>
                  <span className="tabular-nums">
                    {fmtTokens(embeddingType.totalTokens)}
                    <span className="ml-1 text-muted-foreground">/ {fmt(embeddingType.jobsWithTokens)} jobów</span>
                  </span>
                </div>
              )}
              {reportType && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="tabular-nums">
                    ...
                    <span className="ml-1 text-muted-foreground">/ {fmt(reportType.jobsWithTokens)} jobów</span>
                  </span>
                </div>
              )}
            </div>
...
export function DashboardPage() {
  ...
  const embeddingPct = summary.totalItems > 0
    ? Math.round((summary.embeddedItems / summary.totalItems) * 100)
    : 0;
  const reportPct = summary.totalItems > 0
    ? Math.round((summary.itemsWithReport / summary.totalItems) * 100)
    : 0;
  const isScraperActive = (scraperQueue.counts.active ?? 0) > 0 || (scraperQueue.counts.waiting ?? 0) > 0;
  const isEmbeddingActive = stats.embedding.running > 0 || (progress.queue.active ?? 0) > 0 || (progress.queue.waiting ?? 0) > 0;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Przegląd stanu systemu</p>
        </div>
        <div className="flex items-center gap-2">
        ...
        <Card className="lg:col-span-2">
          <CardHeader className="pb-3">
            <CardTitle className="flex items-center gap-2 text-sm font-semibold">
              ...
              {isEmbeddingActive && (
                <Badge variant="default" className="ml-auto animate-pulse text-xs">
                  Aktywny
                </Badge>
              )}
            </CardTitle>
          ...
          <CardContent className="space-y-4">
            <div>
              <div className="mb-1.5 flex items-center justify-between text-xs">
                ...
                <span className="tabular-nums font-medium">
                  ...
                  <span className="ml-1 text-muted-foreground">({embeddingPct}%)</span>
                </span>
              </div>
            ...
            <div>
              <div className="mb-1.5 flex items-center justify-between text-xs">
                <span className="text-muted-foreground">Raporty analityczne</span>
                <span className="tabular-nums font-medium">
                  {fmt(summary.itemsWithReport)} / {fmt(summary.totalItems)}
                  <span className="ml-1 text-muted-foreground">({reportPct}%)</span>
                </span>
              </div>
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-1.5 text-xs sm:grid-cols-4">
              <div>
                ...
                <p className="font-medium tabular-nums">{fmt(summary.pendingItems)}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Błędy</p>
                <p className={`font-medium tabular-nums ${summary.errorItems > 0 ? "text-destructive" : ""}`}>
                  {fmt(summary.errorItems)}
                </p>
              </div>
              <div>
                <p className="text-muted-foreground">Kolejka – aktywne</p>
                <p className="font-medium tabular-nums">{fmt(progress.queue.active)}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Kolejka – waiting</p>
                <p className="font-medium tabular-nums">{fmt(progress.queue.waiting)}</p>
              </div>
            </div>
            ...
            {progress.bySource.length > 0 && (
              <>
                ...
                <div className="space-y-2">
                  <p className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Według źródła</p>
                  {progress.bySource.map((src) => (
                    <div key={src.sourceSystem} className="flex items-center gap-3 text-xs">
                      <span className="w-40 shrink-0 truncate text-muted-foreground">{src.sourceSystem.replace(/_/g, " ")}</span>
                      <Progress
                        value={src.totalItems > 0 ? Math.round((src.embeddedItems / src.totalItems) * 100) : 0}
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-2 gap-x-6 gap-y-2 text-xs sm:grid-cols-4">
              ...
              <div>
                ...
                <p className="font-mono">{scraperQueue.schedule?.[0]?.cron ?? "—"}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Ukończone runy</p>
                <p className="font-medium tabular-nums">{fmt(scraperQueue.counts.completed)}</p>
              </div>
              <div>
                <p className="text-muted-foreground">Śr. czas runu</p>
                <p className="font-medium">{fmtDuration(stats.scraper.avgDurationMs)}</p>
              </div>
            </div>
            ...
            {stats.scraper.lastRunAt && (
              <>
                <Separator />
                <p className="text-xs text-muted-foreground">
                  Ostatni run: <span className="text-foreground">{fmtDate(stats.scraper.lastRunAt)}</span>
                </p>
              </>
            )}
          </CardContent>
